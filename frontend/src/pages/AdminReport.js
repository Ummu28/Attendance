...
const AdminReport = () => {
    ...
    const getEmployee = async () => {
        try {
            axios.get(`http://localhost:5010/api/employees/find/${id}`).then(data => {
    ...
    const getAttendances = async (value) => {
        try {
            ...
            axios.post(`http://localhost:5010/api/attendances/employee/${id}`, { thismonth: value == 1 ? previousMonth : currentMonth })
                .then(data => {
